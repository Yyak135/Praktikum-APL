...
string studioKelas[2] = {"Kelas A", "Kelas B"};
...
{
    daftarReservasi[jumlahReservasi++] = {"ipeh", "202101", "Kelas A", 3, 3 * hargaPerJam[0]};
    daftarReservasi[jumlahReservasi++] = {"aryo", "202102", "Kelas B", 2, 2 * hargaPerJam[1]};
    daftarReservasi[jumlahReservasi++] = {"araa", "202103", "Kelas A", 5, 5 * hargaPerJam[0]};
    daftarReservasi[jumlahReservasi++] = {"inun", "202104", "Kelas B", 4, 4 * hargaPerJam[1]};
    return "HELLO\n";
}
...
string tampilkanReservasiUser(string nimUser) 
{
    ...
    for (int i = 0; i < jumlahReservasi; i++) 
    {
        ...
        {
            adaReservasi = true;
            output += "| " + to_string(i + 1) + "  | " + daftarReservasi[i].kelasStudio + " |  " +
                      to_string(daftarReservasi[i].jam) + "  | Rp " +
                      to_string(daftarReservasi[i].totalHarga) + "         |\n";
        }
    }
    if (!adaReservasi) output += "|          Tidak ada reservasi           |\n";
...
string tampilkanDaftarStudio() 
{
    ...
    for (int i = 0; i < 2; i++) 
    {
        output += "| " + to_string(i + 1) + "  | " + studioKelas[i] + " | ";
        for (int j = 0; j < 7; j++) 
        {
            output += alatMusik[i][j];
            if (j < 6) output += ", ";
        }
        output += " |   Rp " + to_string(hargaPerJam[i]) + "    |\n";
    }
    output += "==================================================================================================\n";
    return output;
}
...
string tambahReservasiUser(string nama, string nim) 
{
    ...
    cin >> jam;
    daftarReservasi[jumlahReservasi++] = {nama, nim, studioKelas[pilihan - 1], jam, jam * hargaPerJam[pilihan - 1]};
...
string editReservasiUser(string nim) 
{
    ...
    cout << tampilkanReservasiUser(nim);
    ...
    daftarReservasi[index - 1].kelasStudio = studioKelas[pilihan - 1];
    cout << "Masukkan jumlah jam baru: ";
    cin >> jam;
    daftarReservasi[index - 1].jam = jam;
    daftarReservasi[index - 1].totalHarga = jam * hargaPerJam[pilihan - 1]; 
...
string tampilkanReservasiAdmin() 
{
    ...
    for (int i = 0; i < jumlahReservasi; i++) 
    {
        output += "| " + to_string(i + 1) + "  | " + daftarReservasi[i].nama + "                 | " +
                  daftarReservasi[i].nim + "    | " + daftarReservasi[i].kelasStudio + " |  " +
                  to_string(daftarReservasi[i].jam) + "  | Rp " +
                  to_string(daftarReservasi[i].totalHarga) + "   |\n";
    }
    output += "=======================================================================\n";
